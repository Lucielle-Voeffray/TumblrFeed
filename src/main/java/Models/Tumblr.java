...
public class Tumblr implements supervisor {
    ...
    public void start() {
        ...
        while (true) {
            ...
            ResultSet results = sql.select("SELECT search.hashtagName, channel.id, user.hashedName FROM t_search AS search JOIN t_channel as channel ON channel.pk_channel = search.fk_channel JOIN t_user AS user ON user.pk_user = search.fk_user WHERE search.paused = false");
            Discord discord = supervisor.getDiscord();
            List<Post> search;

            try {
                while (results.next()) {
                    String channelID = results.getString("id");
                    String hashedName = results.getString("hashedName");
                    String hashtagName = results.getString("hashtagName");
                ...
                Error.report(LogType.ERROR, "Tumblr.java", "start()", 0, e);
            }
    ...
    private @NotNull List<Post> getNewPosts(@NotNull List<Post> posts, String hashedUser, String searchName) {
        ...
        try {
            lastPost = supervisor.getSql().select(String.format("SELECT lastSharedPost FROM t_search WHERE searchName = %s AND fk_user = (SELECT pk_user FROM t_user = %s)", searchName, hashedUser)).getString("lastSharedPost");
        } catch (SQLException e) {
            Error.report(LogType.ERROR, "Tumblr.java", "getNewPosts()", 0, e);
        }
